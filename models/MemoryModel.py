...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.src_memory_encoder=nn.Linear(self.node_feat_dim,self.node_feat_dim)
        self.dst_memory_encoder=nn.Linear(self.node_feat_dim,self.node_feat_dim)
        self.edge_encoder=nn.Linear(self.edge_feat_dim,self.edge_feat_dim)
        
        #self.channel_attn=nn.Parameter(torch.ones(4),requires_grad=True)
        ...
        #self.message_dim =4 * self.time_feat_dim
        self.message_dim = self.memory_dim + self.memory_dim + self.time_feat_dim + self.edge_feat_dim
        
        self.channel_attention = ChannelAttentionTemporal(4)
    ...
    def compute_new_node_raw_messages(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, dst_node_embeddings: torch.Tensor,
        ...
        #src_node_memories=self.src_memory_encoder(src_node_memories)
        #dst_node_memories=self.dst_memory_encoder(dst_node_memories)
        edge_features=self.edge_encoder(edge_features)
        
        #exp_score=[torch.exp(i) for i in self.channel_attn]

        #attn_score=[i/sum(exp_score) for i in exp_score]
        ...
        #new_src_node_raw_messages = torch.cat([src_node_memories*attn_score[0], dst_node_memories*attn_score[1], src_node_delta_time_features*attn_score[2], edge_features*attn_score[3]], dim=1)
...
class ChannelAttentionTemporal(nn.Module):
    def __init__(self, feat_dim, ratio=16):
        super(ChannelAttentionTemporal, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.max_pool = nn.AdaptiveMaxPool1d(1)
        self.fc1 = nn.Linear(feat_dim, feat_dim // ratio, bias=False)
        self.relu1 = nn.ReLU()
        self.fc2 = nn.Linear(feat_dim // ratio, feat_dim, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # x: [batch_size, feat_dim, seq_len]
        avg_out = self.fc2(self.relu1(self.fc1(self.avg_pool(x))))
        max_out = self.fc2(self.relu1(self.fc1(self.max_pool(x))))
        out = avg_out + max_out
        attention_value = attention_value.repeat(1, x.size(-1))
