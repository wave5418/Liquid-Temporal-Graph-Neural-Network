...
from models.modules import LiquidMergeLayer
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        elif args.model_name in ['Liquid']:
            # four floats that represent the mean and standard deviation of source and destination node time shifts in the training data, which is used for JODIE
            src_node_mean_time_shift, src_node_std_time_shift, dst_node_mean_time_shift_dst, dst_node_std_time_shift = \
                compute_src_dst_node_time_shifts(train_data.src_node_ids, train_data.dst_node_ids, train_data.node_interact_times)
            dynamic_backbone = Liquid(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                           time_feat_dim=args.time_feat_dim, model_name=args.model_name, num_layers=args.num_layers, num_heads=args.num_heads,
                                           dropout=args.dropout, src_node_mean_time_shift=src_node_mean_time_shift, src_node_std_time_shift=src_node_std_time_shift,
                                           dst_node_mean_time_shift_dst=dst_node_mean_time_shift_dst, dst_node_std_time_shift=dst_node_std_time_shift, device=args.device,structure_dim=node_raw_features.shape[1])
        else:
            raise ValueError(f"Wrong value for model_name {args.model_name}!")
        if args.model_name == 'Liquid':
            link_predictor = MergeLayer(input_dim1=node_raw_features.shape[1], input_dim2=node_raw_features.shape[1],
                                            hidden_dim=node_raw_features.shape[1], output_dim=1)
        elif args.model_name in ['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'DyGFormer']:
            link_predictor = MergeLayer(input_dim1=node_raw_features.shape[1], input_dim2=node_raw_features.shape[1],
                                    hidden_dim=node_raw_features.shape[1], output_dim=1)
        else:
            raise ValueError(f"Wrong value for model_name {args.model_name}!")
        ...
        coptimizer = torch.optim.Adam(model.parameters(), lr=0.0002)
        cscheduler =  torch.optim.lr_scheduler.CosineAnnealingLR(optimizer = coptimizer,
                                                        T_max =  args.num_epochs) #  * iters 

        clr = [] 
        for epoch in range(args.num_epochs):

            model.train()
            if args.model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer','Liquid']:
                # training, only use training graph
                model[0].set_neighbor_sampler(train_neighbor_sampler)
            if args.model_name in ['JODIE', 'DyRep', 'TGN','Liquid']:
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name == 'Liquid':
                    # src_structure_embedding = batch_co_occur_encoding[:batch_src_node_embeddings.shape[0],:]
                    # dst_structure_embedding = batch_co_occur_encoding[batch_src_node_embeddings.shape[0]:,:]

                    # neg_src_structure_embedding = batch_neg_co_occur_encoding[:batch_src_node_embeddings.shape[0],:]
                    # neg_dst_structure_embedding = batch_neg_co_occur_encoding[batch_src_node_embeddings.shape[0]:,:]

                    feat_positive_probabilities = model[1](input_1=batch_src_node_embeddings, input_2=batch_dst_node_embeddings).squeeze(dim=-1).sigmoid()
                    feat_negative_probabilities = model[1](input_1=batch_neg_src_node_embeddings, input_2=batch_neg_dst_node_embeddings).squeeze(dim=-1).sigmoid()

                    # structure_positive_probabilities = model[1](input_1=src_structure_embedding, input_2=dst_structure_embedding).squeeze(dim=-1).sigmoid()
                    # structure_negative_probabilities = model[1](input_1=neg_src_structure_embedding, input_2=neg_dst_structure_embedding).squeeze(dim=-1).sigmoid()
                elif args.model_name in ['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'DyGFormer']:
                ...
                if args.model_name == 'Liquid':
                    predicts = torch.cat([feat_positive_probabilities,feat_negative_probabilities], dim=0)
                ...
                labels = torch.cat([torch.ones_like(feat_positive_probabilities), torch.zeros_like(feat_negative_probabilities)], dim=0)
                
                if args.model_name == 'Liquid':
                    # cos1 = torch.nn.functional.cosine_similarity(src_structure_embedding, batch_src_node_embeddings, dim=-1).mean() 
                    # cos2 = torch.nn.functional.cosine_similarity(dst_structure_embedding, batch_dst_node_embeddings, dim=-1).mean() 
                    # loss = loss_func(input=predicts, target=labels) + args.lamda * (2 - torch.exp(cos1-1) - torch.exp(cos2-1))
                ...
                #optimizer.step()
                coptimizer.step()
                clr.append(cscheduler.get_lr()[0])
                
                #print(cscheduler.get_lr()[0],coptimizer.param_groups[0]["lr"])
            ...
            logger.info(f'Epoch: {epoch + 1}, learning rate: {coptimizer.param_groups[0]["lr"]}, train loss: {np.mean(train_losses):.4f}')
        ...
        if args.model_name not in ['JODIE', 'DyRep', 'TGN','Liquid']:
            val_losses, val_metrics = evaluate_model_link_prediction(model_name=args.model_name,
                                                                     model=model,
                                                                     neighbor_sampler=full_neighbor_sampler,
                                                                     evaluate_idx_data_loader=val_idx_data_loader,
